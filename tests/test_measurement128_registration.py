...
HERE = Path(__file__).resolve().parents[1] / "specs/128-format-admission-diagnostic/measurement"
...
def modules(monkeypatch):
    ...
    for name in ("case", "observation", "supervision", "campaign", "analysis"):
        spec = importlib.util.spec_from_file_location("measurement128_registration_" + name, HERE / f"{name}.py")
...
def registered(modules, monkeypatch, tmp_path):
    ...
    c.git("-c", "user.name=Offline128", "-c", "user.email=offline128@example.invalid",
...
def test_fixed_native_diagnostic_preserves_wire_roles_and_separate_denominator(modules):
    ...
    assert value["product_commit"] == "b9518570a18e25ad784d00ba864a18ee8e30ac80"
    ...
    assert value["campaign_id"] == "diagnostic128-glm-5.2-format-admission-20260917"
    ...
    assert "Synthetic input format admission" in prompt
    assert "JSON roots must be an object or an array of objects" in prompt
    assert c.REFERENCE.relative_to(c.REPO).as_posix() in c.pins(c.historical_paths())
    for path in ("specs/125-snapshot-protocol-diagnostic/postrun/results.json",
                 "specs/126-synthetic-input-format/spec.md",
                 "specs/127-evaluator-preparation-diagnostics/spec.md"):
        assert path in c.pins(c.historical_paths())
    assert value["local_failure_capture"] == {
...
@pytest.mark.parametrize("field", [
    ...
    "previous_campaign", "condition_changes", "local_failure_capture",
])
