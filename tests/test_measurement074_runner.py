...
@pytest.fixture
def adapter():
    spec = importlib.util.spec_from_file_location("offline_feature074_adapter", ADAPTER)
...
def test_changed_pinned_helper_is_rejected_before_evaluation(tmp_path, adapter, monkeypatch):
    ...
    monkeypatch.setattr(adapter._loader, "LEGACY_ROOT", root)
...
def test_prior_helper_closure_is_pinned_and_private(adapter, tmp_path, monkeypatch):
    assert len(adapter.HELPER_FILES) == 12
    for relative, expected in adapter.HELPER_FILES.items():
        assert hashlib.sha256((ROOT / relative).read_bytes()).hexdigest() == expected
    prior = adapter.load_prior_postrun()
    assert prior.__file__.endswith("072-master-budget-measurement/postrun/audit.py")
    assert callable(prior.stage_evidence)
    root = tmp_path / "prior"
    (root / "measurement").mkdir(parents=True)
    (root / "measurement/audit.py").write_text("raise AssertionError('must not execute')")
    monkeypatch.setattr(adapter, "PRIOR_ROOT", root)
    with pytest.raises(ValueError, match="helper changed"):
        adapter.load_prior("audit")
...
def scaffold(tmp_path, adapter, monkeypatch):
    ...
    manifest = {
        "campaign_id": campaign.name, "planned_attempts": 2, "execution": copy.deepcopy(adapter.EXECUTION),
...
def test_two_exact_bindings_use_staged_commands_and_shared_ceilings(tmp_path, adapter, monkeypatch):
    ...
    assert manifest["policies"] == adapter.POLICIES
...
@pytest.mark.parametrize("alteration", ["third_slot", "third_cli_index", "extra_wave", "four_planned"])
def test_no_extra_registered_or_cli_slot_can_dispatch(tmp_path, adapter, monkeypatch, alteration):
    worker, _, campaign, manifest, digest, _, _ = scaffold(tmp_path, adapter, monkeypatch)
    if alteration == "third_slot":
        manifest["slots"].append({**manifest["slots"][0], "index": 3})
    elif alteration == "extra_wave":
        manifest["execution"]["waves"].append([3, 4])
    elif alteration == "four_planned":
        manifest["planned_attempts"] = 4
    path = campaign / "manifest.json"
    path.write_bytes(_canonical_bytes(manifest))
    digest = worker.file_sha256(path)
    (campaign / "manifest.sha256").write_text(digest + "\n")
    if alteration == "third_cli_index":
        monkeypatch.setattr(sys, "argv", ["worker.py", "--manifest", str(path), "--campaign", str(campaign), "--slot", "3"])
        with pytest.raises(EffectTrialError):
            worker.main()
        assert not (campaign / "slots").exists()
    else:
        with pytest.raises(EffectTrialError):
            worker.verify_bindings(path, campaign, manifest, digest, manifest["slots"][0], require_started=False)
...
class Subject(StagedSubject):
    ...
    def complete(self, messages, tools=(), timeout=None):
        ...
        turn = replace(super().complete(messages, tools, timeout), response_model="glm-5.2")
        if self.turn == 1 and self.scenario == "fenced_success":
            turn = replace(turn, text="Public data inspected.\n```json\n" + turn.text + "\n```\nReady for Build.")
        return turn


@pytest.mark.parametrize("index", [1, 2])
@pytest.mark.parametrize("scenario", ["success", "fenced_success", "master_timeout", "invalid_plan", "failed_extraction"])
def test_native_staged_subject_and_receipt_gate(tmp_path, adapter, monkeypatch, index, scenario):
    ...
    success = scenario in {"success", "fenced_success", "failed_extraction"}
    ...
    assert row["overall_score"] == (1.2 if scenario in {"success", "fenced_success"} else None)
    assert summary["budget_arms"][slot["budget_arm"]]["valid_solution_rate"] == (0.5 if scenario in {"success", "fenced_success"} else 0)
    assert summary["planned_attempts"] == 2 and not summary["all_slots_terminated"]
...
def test_single_wave_dispatches_only_two_slots_and_markers_are_exclusive(tmp_path, adapter, monkeypatch):
    campaign = adapter.load_campaign()
    calls, lock, rendezvous = [], threading.Lock(), threading.Barrier(2)
    ...
    assert campaign.execute_waves({"execution": {"waves": [[1, 2]]},
                                  "slots": [{"index": i} for i in (1, 2)]}, {}, "a" * 64) == [1, 2]
