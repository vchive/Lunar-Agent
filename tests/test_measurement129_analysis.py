...
import pytest
from test_measurement129_case import compiled
...
def modules(monkeypatch):
    ...
    for name in ("case", "observation", "supervision", "campaign", "analysis"):
        spec = importlib.util.spec_from_file_location("measurement129_analysis_" + name, HERE / f"{name}.py")
...
def fixture(modules, slot, monkeypatch, *, value=3):
    ...
    events = [{"type": "bundle_profile_prepared"}, {"type": "bundle_candidate_delivered", "payload": terminal}]
...
def test_usage_accepts_full_native_sequence_and_binds_transport_stages(modules, tmp_path):
    ...
    row = {"request_index": 4, "request_sha256": request["request_sha256"], "stage": "candidate",
           "outcome": "response", "elapsed_ms": 10, "transport_observation": None, "private": "must-stay-private"}
...
def retained_campaign(modules, tmp_path, monkeypatch, *, prepared=True):
    ...
    manifest = {"campaign_root": ".lunar/offline129", "product_commit": "a" * 40}
    ...
    if prepared:
        ...
        c.write_new(slot / "worker-finished.json", {"status": "completed", "stage": "finished",
