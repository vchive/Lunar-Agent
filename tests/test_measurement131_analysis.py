...
import pytest
from test_measurement131_case import compiled
...
def modules(monkeypatch):
    ...
    for name in ("case", "observation", "supervision", "campaign", "analysis"):
        spec = importlib.util.spec_from_file_location("measurement131_analysis_" + name, HERE / f"{name}.py")
...
def retained_campaign(modules, tmp_path, monkeypatch, *, prepared=True):
    ...
    manifest = {"campaign_root": ".lunar/offline131", "product_commit": "a" * 40}
