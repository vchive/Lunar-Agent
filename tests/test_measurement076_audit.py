"""Rebind the sealed audit fixtures and independently check 076 paths and helper closure."""
import hashlib
...
_spec = importlib.util.spec_from_file_location("offline076_audit_adapter", ADAPTER)
...
def test_prior_audit_is_privately_bound_without_changing_four_slot_module(adapter, audit):
    ...
    assert audit.native.IMPLEMENTATION == "96d5a600f03e5401753efc6f579d2ff14fe839be"
...
def freeze_fixture(tmp_path, monkeypatch, audit):
    here = tmp_path / "specs/076-public-plan-handoff-measurement/measurement"
    ...
    paths = {*(here.parent / name for name in audit.REQUIRED_CODE),
             *(tmp_path / relative for relative in audit.EXPECTED_HELPERS),
             tmp_path / "tests/test_measurement076_audit.py", tmp_path / "tests/test_measurement076_runner.py",
...
def test_dry_run_uses_new_scenarios_and_freezes_all_test_bytes(adapter):
    ...
    assert {"tests/test_measurement076_runner.py", "tests/test_measurement076_audit.py",
            "tests/test_master_plan_envelope_integration.py", "tests/test_master_plan_vocabulary.py",
            "specs/076-public-plan-handoff-measurement/postrun/test_audit.py"} <= set(dry.NODES)
...
def test_new_audit_default_and_explicit_paths_reject_old_registration(audit, adapter, tmp_path):
    assert audit.audit.__kwdefaults__["repo"] == adapter.REPO
    with pytest.raises(audit.AuditError, match="registration paths"):
        audit.audit(adapter.PREVIOUS_ROOT / "measurement/manifest.json", adapter.CAMPAIGN)
    with pytest.raises(audit.AuditError, match="registration paths"):
        audit.audit(tmp_path / "specs/076-public-plan-handoff-measurement/measurement/manifest.json",
                    tmp_path / ".lunar/real-eval-glm-5.2-corrected-staged-20260910", repo=tmp_path)


def test_history_uses_sealed_074_chain_and_keeps_historical_samples_out(audit, tmp_path, monkeypatch):
    assert audit.PRIOR_ROOT == "specs/074-corrected-staged-measurement"
    assert audit.SEALED_HISTORY[audit.PRIOR_ROOT + "/measurement/manifest.json"] == (
        "b154975d9557b9697fcdc7915de3ab7bb225165c6aaa1c6c0ab3f432a2123229"
    )
