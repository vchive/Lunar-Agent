...
HERE = Path(__file__).resolve().parents[1] / "specs/123-small-evaluator-diagnostic/measurement"
...
def modules(monkeypatch):
    ...
    for name in ("case", "observation", "supervision", "campaign", "analysis"):
        spec = importlib.util.spec_from_file_location("measurement123_registration_" + name, HERE / f"{name}.py")
...
def test_fixed_native_diagnostic_preserves_wire_roles_and_separate_denominator(modules):
    ...
    assert value["product_commit"] == "519fea5ca70ac1ede3df356ee7391112801ab45d"
    ...
    assert value["provider"] == json.loads(c.REFERENCE.read_text())["provider"]
    assert 10000 < value["compiler_request"]["bytes"] < 20000
    assert len(value["compiler_request"]["sha256"]) == 64
...
def test_offline_registration_is_once_only_and_verifies_without_provider_requests(registered):
    ...
    assert "specs/123-small-evaluator-diagnostic/measurement/analysis.py" in manifest["measurement_files"]
    assert len(manifest["historical_files"]) >= 64
