"""Offline accounting, typed native failures and passive transport observation."""
...
from famou.agents import AgentRequest, CandidateGenerationBudget, RuntimeAgentAdapter
...
HELPER = ROOT / "specs/139-real-multifile-closure/measurement/observation.py"
spec = importlib.util.spec_from_file_location("measurement139_observation_test", HELPER)
...
def test_loader_uses_unique_module_without_search_path_mutation():
    ...
    assert shared.__name__ == "_lunar_measurement139_frozen113_runtime_guard"
...
def test_success_payload_sequence_hashes_and_legacy_ledger_are_unchanged(
    ...
    assert [row["stage"] for row in rows] == ["unbound", "unbound"]
    assert [row["context_bound"] for row in rows] == [False, False]
    for row in rows:
...
def test_http_error_body_is_only_hashed_and_never_captured_as_assistant_text(
    ...
    monkeypatch.setattr(runtime_module, "exchange", exchange)
    guard = make_guard(tmp_path, provider)
    with module.observed_guard(guard), pytest.raises(ModelRequestFailure):
...
def test_exact_native_failure_survives_only_after_identical_durable_accounting(tmp_path, provider, monkeypatch):
    failure = ModelRequestFailure("private native failure", "transport_timeout")
    failure.observation = ModelRequestObservation("open_response", 900000, 900000)
    failure.transport_observation = TransportObservation("wait_response_headers", 1, 900000)
    calls = []
    ...
    monkeypatch.setattr(OpenAICompatibleRuntime, "complete", native)
    fixed_clock = lambda: 100.0
    observed = make_guard(tmp_path / "observed", provider, clock=fixed_clock, request_seconds=900)
    legacy = module.BaseRuntimeGuard(provider, MODEL, tmp_path / "legacy/calls.jsonl",
                                     clock=fixed_clock, request_seconds=900)
    for guard, context in ((observed, module.observed_guard), (legacy, module._shared.installed_guard)):
        with context(guard):
            runtime = OpenAICompatibleRuntime()
            if guard is observed:
                with pytest.raises(ModelRequestFailure) as caught:
                    runtime.complete(MESSAGES, timeout=900)
                assert caught.value is failure
                assert caught.value.observation is failure.observation
                assert read_rows(guard.journal_path)[-1]["kind"] == "request_finished"
            else:
                with pytest.raises(module.MeasurementRuntimeError, match="provider_error"):
                    runtime.complete(MESSAGES, timeout=900)
            with pytest.raises(module.MeasurementRuntimeError, match="provider_error"):
    ...
    assert len(calls) == 2
    assert observed.journal_path.read_bytes() == legacy.journal_path.read_bytes()
    assert observed.snapshot()["finished_requests"] == 1
    assert observed.snapshot()["usage_complete"] is False
    assert "private native failure" not in observed.journal_path.read_text()


@pytest.mark.parametrize("phase", ["start", "finish"])
def test_journal_failure_keeps_precedence_over_native_failure(tmp_path, provider, monkeypatch, phase):
    failure = ModelRequestFailure("private native failure", "transport_timeout")
    calls = []
    ...
    def native(*args):
        calls.append(1)
        if phase == "finish":
            guard.journal_path.unlink()
            guard.journal_path.mkdir()
        raise failure
    ...
    if phase == "start":
        ...
        guard.journal_path.mkdir()
    with module.observed_guard(guard):
        with pytest.raises(module.MeasurementRuntimeError, match="request_journal_unavailable"):
            OpenAICompatibleRuntime().complete(MESSAGES)
        with pytest.raises(module.MeasurementRuntimeError, match="request_journal_unavailable"):
            OpenAICompatibleRuntime().complete(MESSAGES)
    assert len(calls) == (1 if phase == "finish" else 0)
    assert guard.snapshot()["stopped_reason"] == "request_journal_unavailable"


def test_native_failure_subclass_is_not_promoted_to_owned_diagnostic(tmp_path, provider, monkeypatch):
    class ForeignFailure(ModelRequestFailure):
        pass

    def native(*args):
        raise ForeignFailure("private foreign failure", "transport_timeout")

    monkeypatch.setattr(OpenAICompatibleRuntime, "complete", native)
    guard = make_guard(tmp_path, provider)
    with module.observed_guard(guard), pytest.raises(module.MeasurementRuntimeError, match="provider_error"):
        OpenAICompatibleRuntime().complete(MESSAGES)
    assert guard.snapshot()["finished_requests"] == 1


def test_candidate_agent_request_binds_every_nested_model_turn(tmp_path, provider, monkeypatch):
    monkeypatch.setattr(runtime_module, "exchange", lambda request, timeout: response())

    class MultiTurnRuntime:
        name = "multi-turn-fixture"

        def run(self, prompt, workspace, timeout=None, **kwargs):
            runtime = OpenAICompatibleRuntime()
            runtime.complete(MESSAGES, timeout=timeout)
            runtime.complete([*MESSAGES, {"role": "assistant", "content": "tool used"}], timeout=timeout)
            return RuntimeResult("candidate")
    ...
    guard = make_guard(tmp_path, provider, max_requests=3)
    adapter = RuntimeAgentAdapter(MultiTurnRuntime())
    request = AgentRequest(
        run_id="run-001", task_id="task-001", role="solver", prompt="make a candidate",
        workspace=tmp_path.resolve(), timeout=30,
        candidate_budget=CandidateGenerationBudget("budget-001", 12, 30),
    )
    with module.observed_guard(guard):
        assert adapter.run(request).text == "candidate"
        with pytest.raises(RuntimeError, match="duplicate_native_candidate_budget"):
            adapter.run(request)
    rows = sidecar(guard)
    assert len(rows) == 2
    assert all(row["context_bound"] is True for row in rows)
    assert all(row["stage"] == "candidate_generation" for row in rows)
    assert all(row["run_id"] == "run-001" and row["task_id"] == "task-001" for row in rows)
    assert all(row["budget_id"] == "budget-001" and row["max_tool_steps"] == 12 for row in rows)
    bound = guard.native_requests["budget-001"]
    assert bound["run_id"] == request.run_id and bound["task_id"] == request.task_id
    assert 0 <= bound["started_at"] <= bound["finished_at"]


@pytest.mark.parametrize("stage,ceiling", [
    (None, 600), ("contract_compiler", 600), ("evaluator_compiler", 900),
    ("evaluator_auditor", 900), ("candidate_generation", 600),
])
@pytest.mark.parametrize("remaining,requested", [(2400, 2000), (450, 900), (2400, None), (2400, 300)])
def test_preparation_request_ceiling_keeps_ordinary_and_deadline_bounds(
    tmp_path, provider, monkeypatch, stage, ceiling, remaining, requested,
):
    seen = []
    ...
    monkeypatch.setattr(runtime_module, "exchange", exchange)
    guard = make_guard(
        tmp_path, provider, clock=lambda: 100.0, wall_seconds=remaining,
        request_seconds=600, preparation_request_seconds=900,
    )
    identity = {"run_id": "run-001", "task_id": "task-001", "budget_id": "budget-001",
                "max_tool_steps": 12} if stage == "candidate_generation" else {}
    context = nullcontext() if stage is None else guard.request_context(stage, **identity)
    with module.observed_guard(guard), context:
        assert OpenAICompatibleRuntime().complete(MESSAGES, timeout=requested).text == "native answer"
    effective = min(remaining, ceiling, requested if requested is not None else ceiling)
    assert seen == [effective]
    assert read_rows(guard.journal_path)[0]["request_timeout_seconds"] == effective
    assert guard.request_seconds == 600


def test_preparation_ceiling_does_not_leak_between_threads(tmp_path, provider, monkeypatch):
    monkeypatch.setattr(runtime_module, "exchange", lambda *_: response())
    guard = make_guard(
        tmp_path, provider, clock=lambda: 100.0, wall_seconds=2400,
        request_seconds=600, preparation_request_seconds=900,
    )
    barrier = Barrier(2)
    ...
    def request(stage):
        with guard.request_context(stage):
            ...
            ceiling = guard.request_seconds
            OpenAICompatibleRuntime().complete(MESSAGES, timeout=2000)
            return stage, ceiling

    with module.observed_guard(guard), ThreadPoolExecutor(max_workers=2) as executor:
        futures = [executor.submit(request, stage) for stage in ("contract_compiler", "evaluator_compiler")]
        assert dict(future.result(timeout=5) for future in futures) == {
            "contract_compiler": 600, "evaluator_compiler": 900,
        }
    starts = {row["index"]: row for row in read_rows(guard.journal_path) if row["kind"] == "request_started"}
    observed = {row["stage"]: starts[row["request_index"]]["request_timeout_seconds"] for row in sidecar(guard)}
    assert observed == {"contract_compiler": 600, "evaluator_compiler": 900}


@pytest.mark.parametrize("timeout", [0, -1, True, float("nan"), float("inf"), "900"])
def test_invalid_preparation_timeout_rejected_without_provider(tmp_path, provider, timeout):
    with pytest.raises(ValueError, match="invalid_preparation_request_timeout"):
        make_guard(tmp_path, provider, preparation_request_seconds=timeout)
