...
import pytest
from test_measurement128_case import SOURCE, suite
...
HERE = (Path(__file__).resolve().parents[1]
        / "specs/128-format-admission-diagnostic/measurement")
...
def modules(monkeypatch, tmp_path):
    case = load(monkeypatch, "measurement128_runner_case", "case.py")
    observation = load(monkeypatch, "measurement128_runner_observation", "observation.py")
    supervision = load(monkeypatch, "measurement128_runner_supervision", "supervision.py")
    for name, module in (("case", case), ("observation", observation), ("supervision", supervision)):
        monkeypatch.setitem(sys.modules, name, module)
    campaign = load(monkeypatch, "measurement128_runner_campaign", "campaign.py")
...
def test_run_claims_one_new_slot_before_launch_and_never_reopens_it(modules):
    ...
    old_roots = [campaign.REPO / ".lunar" / name for name in (
        "acceptance120-frozen", "diagnostic123-glm-5.2-small-evaluator-20260917",
    )]
    for old in old_roots:
        old.mkdir(parents=True)
        (old / "started.json").write_bytes(b"old immutable evidence")
        (old / "finished.json").write_bytes(b"old immutable outcome")
    calls = []
    ...
    for old in old_roots:
        ...
        assert (old / "finished.json").read_bytes() == b"old immutable outcome"
...
@pytest.mark.parametrize("outcome,expected_calls,expected_processes", [
    ("success", 2, 14), ("compiler_json", 1, 0), ("compiler_source", 1, 0),
    ("compiler_preflight", 1, 2), ("compiler_input_path", 1, 1),
    ("compiler_timeout", 1, 0), ("auditor_json", 2, 3),
    ("compiler_input_format", 1, 0), ("auditor_input_format", 2, 3),
    ("auditor_validity", 2, 4), ("auditor_report", 2, 4),
])
def test_worker_native_isolation_limits_and_conditional_auditor(
    modules, monkeypatch, capsys, outcome, expected_calls, expected_processes,
):
    campaign, case, observation, _, manifest, provider = modules
    _, slot = start_worker(campaign, manifest)
    worker = load(monkeypatch, "measurement128_runner_worker", "worker.py")
    verifications, provider_calls, native_calls, guards = [], [], [], []
    processes = []
    real_process = candidate_execution_runner._bounded_process_bytes

    def inspect_process(command, **kwargs):
        root = Path(kwargs["cwd"])
        request = json.loads((root / "request.json").read_bytes())
        descriptor, = request["inputs"]
        assert set(descriptor) == {"target", "source_label", "size", "sha256"}
        assert descriptor["target"] == "limit.json"
        assert (root / "inputs" / descriptor["target"]).is_file()
        output, = request["outputs"]
        assert set(output) == {"path", "present", "size", "sha256"}
        assert output["path"] == "output/result.json" and output["present"] is True
        assert (root / output["path"]).is_file()
        assert not (root / "output" / output["path"]).exists()
    ...
    def native(runtime, messages, tools=(), timeout=None):
        ...
        assert "Runtime inputs[] has no path field" in messages[1]["content"]
        assert "contract.inputs[].path equals that target" in messages[1]["content"]
        assert "do not prepend output/" in messages[1]["content"]
        if index == 1:
            ...
            if outcome == "compiler_input_path":
                source = source.replace('item.get("target")', 'item.get("path")')
            if outcome == "auditor_validity":
                source = source.replace("valid = type(value) is int and 0 <= value <= limit",
                                        "valid = type(value) is int and 0 <= value <= limit and limit != 4")
            if outcome == "auditor_report":
                source = source.replace('"schema_version": "1"',
                                        '"schema_version": "broken" if limit == 4 else "1"')
            compiler_suite = suite(2)
            if outcome == "compiler_input_format":
                compiler_suite["probes"][0]["files"][0]["content"] = "2"
            text = json.dumps({**compiler_suite, "evaluator_source": source, "objective": "Maximize value."})
            ...
            auditor_suite = suite(4)
            if outcome == "auditor_input_format":
                auditor_suite["probes"][0]["files"][0]["content"] = "4"
            text = "not an audit suite" if outcome == "auditor_json" else json.dumps(auditor_suite)
    ...
    monkeypatch.setattr(candidate_execution_runner, "_bounded_process_bytes", inspect_process)
    ...
    expected_detail = {
        "compiler_json": ("compiler_response", "response_invalid", None, None),
        "compiler_source": ("compiler_response", "response_invalid", None, None),
        "compiler_preflight": ("compiler_preflight", "validity_mismatch", 2, None),
        "compiler_input_path": ("compiler_preflight", "process_failed", 1, None),
        "compiler_input_format": ("compiler_preflight", "input_format_invalid", 1, 1),
        "auditor_json": ("auditor_response", "response_invalid", None, None),
        "auditor_input_format": ("auditor_preflight", "input_format_invalid", 1, 1),
        "auditor_validity": ("auditor_preflight", "validity_mismatch", 1, None),
        "auditor_report": ("auditor_preflight", "report_invalid", 1, None),
    }.get(outcome)
    if expected_detail is None:
        assert result.get("local_failure") is None
    else:
        stage, reason, probe_index, input_index = expected_detail
        assert result["local_failure"] == {
            "schema_version": "1", "stage": stage, "reason": reason,
            "probe_index": probe_index, "input_index": input_index, "order_index": None,
        }
        assert result["error_class"] == "EvaluatorPreparationError"
...
def test_worker_requires_matching_unfinished_attempt_before_provider_access(modules, monkeypatch, invalid):
    ...
    worker = load(monkeypatch, "measurement128_runner_marker_worker", "worker.py")
...
def test_worker_setup_failure_consumes_claim_without_model_call(modules, monkeypatch):
    campaign, _, _, _, manifest, _ = modules
    _, slot = start_worker(campaign, manifest)
    worker = load(monkeypatch, "measurement128_runner_setup_worker", "worker.py")
...
@pytest.mark.parametrize("outcome,expected_calls", [
    ("success", 2), ("compiler_input_format", 1), ("auditor_validity", 2),
])
def test_worker_evidence_is_summarized_once_without_reexecution(
    modules, monkeypatch, tmp_path, outcome, expected_calls,
):
    ...
    worker = load(monkeypatch, "measurement128_success_evidence_worker", "worker.py")
    monkeypatch.setattr(worker, "verify", lambda **kwargs: manifest)
    ...
    def offline_exchange(request, timeout):
        ...
        assert set(wire) == {"model", "messages", "stream"}
        assert wire["model"] == "glm-5.2" and wire["stream"] is False
        assert [item["role"] for item in wire["messages"]] == ["system", "user"]
        assert "Runtime inputs[] has no path field" in wire["messages"][1]["content"]
        assert "do not prepend output/" in wire["messages"][1]["content"]
        if index == 1:
            ...
            if outcome == "auditor_validity":
                source = source.replace("valid = type(value) is int and 0 <= value <= limit",
                                        "valid = type(value) is int and 0 <= value <= limit and limit != 4")
            answer = {**compiler_suite, "evaluator_source": source, "objective": "Maximize value."}
        else:
            answer = suite(4)
    ...
    assert len(exchanges) == expected_calls
    slot = campaign.REPO / manifest["campaign_root"] / "attempt-001"
    assert json.loads((slot / "worker-finished.json").read_text())["status"] == (
        "completed" if outcome == "success" else "failed")
    analysis = load(monkeypatch, "measurement128_success_evidence_analysis", "analysis.py")
    ...
    assert result["preparation_success"] == result["joint_success"] == int(outcome == "success")
    assert result["holdouts"]["planned"] == 8
    assert result["holdouts"]["executed"] == result["holdouts"]["matched"] == (8 if outcome == "success" else 0)
    assert result["usage"]["provider_requests"] == result["usage"]["finished_requests"] == expected_calls
    assert result["usage"]["usage_complete"] is True
    assert result["usage"]["total_usage"] == {
        "input_tokens": 3 * expected_calls, "output_tokens": 2 * expected_calls,
        "total_tokens": 5 * expected_calls,
    }
    assert len(result["transport"]) == expected_calls
    ...
    if outcome == "success":
        ...
        assert result["frozen"] is None and not (slot / "frozen.json").exists()
        assert result["local_failure"] == {
            "schema_version": "1", "stage": "compiler_preflight" if expected_calls == 1 else "auditor_preflight",
            "reason": "input_format_invalid" if expected_calls == 1 else "validity_mismatch",
            "probe_index": 1, "input_index": 1 if expected_calls == 1 else None, "order_index": None,
        }
