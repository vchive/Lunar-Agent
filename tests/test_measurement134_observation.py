...
HELPER = ROOT / "specs/134-budgeted-multifile-acceptance/measurement/observation.py"
spec = importlib.util.spec_from_file_location("measurement134_observation_test", HELPER)
...
def test_loader_uses_unique_module_without_search_path_mutation():
    ...
    assert shared.__name__ == "_lunar_measurement134_frozen113_runtime_guard"
...
def test_success_payload_sequence_hashes_and_legacy_ledger_are_unchanged(
    ...
    assert [row["stage"] for row in rows] == ["contract_compiler", "evaluator_compiler"]
    for row in rows:
...
def test_response_redaction_utf8_limit_and_explicit_private_permissions(
    ...
    assert capture["stage_hint"] == "contract_compiler"
