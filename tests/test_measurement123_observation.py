"""Offline evidence that the 123 observer preserves frozen request accounting and transport."""
...
HELPER = ROOT / "specs/123-small-evaluator-diagnostic/measurement/observation.py"
spec = importlib.util.spec_from_file_location("measurement123_observation_test", HELPER)
...
def test_loader_uses_unique_module_without_search_path_mutation():
    ...
    assert shared.__name__ == "_lunar_measurement123_frozen113_runtime_guard"
...
@pytest.mark.skipif(os.name != "posix", reason="native isolated HTTP requires POSIX")
def test_native_loopback_response_observation_without_real_provider(tmp_path, monkeypatch):
    received = []
    ...
    provider = module.ProviderConfiguration(f"http://127.0.0.1:{server.server_port}/v1", "local-key", MODEL)
    guard = make_guard(tmp_path, provider)
    try:
        with module.observed_guard(guard):
    ...
    assert received[0][:2] == ("/v1/chat/completions", "Bearer local-key")
