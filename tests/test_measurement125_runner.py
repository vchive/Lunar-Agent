...
import pytest
from test_measurement125_case import SOURCE, suite
...
HERE = (Path(__file__).resolve().parents[1]
        / "specs/125-snapshot-protocol-diagnostic/measurement")
...
def modules(monkeypatch, tmp_path):
    case = load(monkeypatch, "measurement125_runner_case", "case.py")
    observation = load(monkeypatch, "measurement125_runner_observation", "observation.py")
    supervision = load(monkeypatch, "measurement125_runner_supervision", "supervision.py")
    for name, module in (("case", case), ("observation", observation), ("supervision", supervision)):
        monkeypatch.setitem(sys.modules, name, module)
    campaign = load(monkeypatch, "measurement125_runner_campaign", "campaign.py")
...
def test_worker_native_isolation_limits_and_conditional_auditor(
    ...
    worker = load(monkeypatch, "measurement125_runner_worker", "worker.py")
...
def test_worker_requires_matching_unfinished_attempt_before_provider_access(modules, monkeypatch, invalid):
    ...
    worker = load(monkeypatch, "measurement125_runner_marker_worker", "worker.py")
...
def test_worker_setup_failure_consumes_claim_without_model_call(modules, monkeypatch):
    campaign, _, _, _, manifest, _ = modules
    _, slot = start_worker(campaign, manifest)
    worker = load(monkeypatch, "measurement125_runner_setup_worker", "worker.py")
...
def test_successful_worker_evidence_is_summarized_once_without_reexecution(
    ...
    worker = load(monkeypatch, "measurement125_success_evidence_worker", "worker.py")
    monkeypatch.setattr(worker, "verify", lambda **kwargs: manifest)
    ...
    analysis = load(monkeypatch, "measurement125_success_evidence_analysis", "analysis.py")
