...
HERE = Path(__file__).resolve().parents[1] / "specs/129-small-multifile-acceptance/measurement"
...
def modules(monkeypatch):
    ...
    for name in ("case", "observation", "supervision", "campaign", "analysis"):
        spec = importlib.util.spec_from_file_location("measurement129_registration_" + name, HERE / f"{name}.py")
...
def registered(modules, monkeypatch, tmp_path):
    ...
    c.git("-c", "user.name=Offline129", "-c", "user.email=offline129@example.invalid",
...
def test_fixed_native_acceptance_preserves_separate_denominator(modules):
    ...
    assert value["limits"] == {"request_seconds": 600, "wall_seconds": 2400, "max_requests": 20,
    ...
    assert value["campaign_id"] == "acceptance129-glm-5.2-small-multifile-20260917"
...
@pytest.mark.parametrize("field", [
    ...
    "reference_preparation", "local_failure_capture", "holdout_continuation", "official_quality_requires",
])
