...
HERE = Path(__file__).resolve().parents[1] / "specs/113-real-multifile-acceptance/measurement"


@pytest.fixture
def campaign(monkeypatch, tmp_path):
    monkeypatch.syspath_prepend(str(HERE))
    spec = importlib.util.spec_from_file_location("measurement113_campaign_runner", HERE / "campaign.py")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    monkeypatch.setitem(sys.modules, "campaign", module)
    local_here = tmp_path / "specs/113/measurement"
...
def manifest(campaign):
    return {
        "campaign_id": "offline-113", "campaign_root": "private/campaign", "product_commit": "fixture",
...
def load_worker(monkeypatch, campaign):
    spec = importlib.util.spec_from_file_location("measurement113_worker_runner", HERE / "worker.py")
...
def verification_fixture(campaign, monkeypatch):
    ...
    measured = ["specs/113/measurement/campaign.py"]
...
def test_verify_requires_each_pinned_source_committed_not_only_manifest(campaign, monkeypatch):
    ...
    blobs["specs/113/measurement/campaign.py"] = b"previous committed bytes"
