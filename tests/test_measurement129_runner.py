...
import pytest
from test_measurement129_case import SOURCE, fixture_contract, suite
...
HERE = (Path(__file__).resolve().parents[1]
        / "specs/129-small-multifile-acceptance/measurement")
...
def modules(monkeypatch, tmp_path):
    case = load(monkeypatch, "measurement129_runner_case", "case.py")
    observation = load(monkeypatch, "measurement129_runner_observation", "observation.py")
    supervision = load(monkeypatch, "measurement129_runner_supervision", "supervision.py")
    for name, module in (("case", case), ("observation", observation), ("supervision", supervision)):
        monkeypatch.setitem(sys.modules, name, module)
    campaign = load(monkeypatch, "measurement129_runner_campaign", "campaign.py")
    ...
    analysis = load(monkeypatch, "measurement129_runner_analysis", "analysis.py")
    monkeypatch.setitem(sys.modules, "analysis", analysis)
    worker = load(monkeypatch, "measurement129_runner_worker", "worker.py")
