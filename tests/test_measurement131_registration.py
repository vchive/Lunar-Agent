...
def modules(monkeypatch):
    ...
    for name in ("case", "observation", "supervision", "campaign", "analysis"):
        spec = importlib.util.spec_from_file_location("measurement131_registration_" + name, HERE / f"{name}.py")
...
def registered(modules, monkeypatch, tmp_path):
    ...
    c.git("-c", "user.name=Offline131", "-c", "user.email=offline131@example.invalid",
...
def test_fixed_native_acceptance_preserves_separate_denominator(modules):
    ...
    assert value["product_commit"] == "c730483080d5f53e5bb2d24e4bb2d2d9f45e984e"
    ...
    assert value["campaign_id"] == "acceptance131-glm-5.2-small-multifile-20260917"
