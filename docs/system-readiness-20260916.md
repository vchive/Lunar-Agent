...
评估日期：2026-09-16，已更新至 Feature 116。最初盘点基于 `af4f8d8`（Feature 107）；
...
1. 113、115 分别完成独立两例 GLM-5.2 验收，均为 0/2。115 有一例通过修复后的合同
   编译，但两例最终被单请求超时阻断。116 已修复 DAG 等待误当用户等待和准备错误
   缺持久事件的问题；下一步考虑独立登记与模型耗时匹配的新预算验收。旧槽不补，
   评测器质量仍未获得真实验收，不因恢复能力增加而声称有效率提高。
