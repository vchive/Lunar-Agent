...
原始注册 24 passed，独立审查通过。产品 `ff1edcb` 的 Linux 3.12/3.13 全过；3.11 暴露
一项旧测试的 SQLite 连接 GC 时序问题，仅测试修复已在两个版本各通过 86 项，完整 CI 待复验。精确结果见
...
前台自动多文件已完成离线产品链路，但真实完整交付仍未成功；包含自动后台与并发 worker
能力的完整发布还必须关闭各自的实现缺口。不能把未接入的可选框架当作所有发布范围的前置条件。
...
| P1：包含自动后台的发布 | 142 Phase C 本机完整验收与独立审查已通过；旧夹具的 GC 竞态已修，Linux CI 待复验 | solve/resume/answer 后台入口、启动认领与退出释放、答案只接收一次、前后台一致性、取消与恢复；最终结果见 142 validation |
