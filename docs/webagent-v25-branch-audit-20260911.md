...
最新参考基线为 v2.5 base，固定提交 `f6ad20caf8963d105fe26c96b6a30ef4dfd34859`。它把远端参考引擎 v2 控制接口接入完整 `opencode` 包，并在配置中对 `evolve_create` 设置 `ask` 审批；`opencode_lite` 则从注册表移除整个 evolve 工具族。仓库构建脚本同时生成完整版、lite 版及兼容别名，因此目录名不能单独证明线上选用了哪一版。

`master`、v2.5 master-agent（描述性标签）、v2.5 evolve_tool（描述性标签） 和 `feature/or-agent` 都与最新 base 分叉，且四个 tip 的主要 `opencode` 演化入口仍是旧的本地 evolution-loop（相同 blob），不能把它们的分支名直接当成最新部署实现。参考引擎服务路由也不是 e24 之后才出现：旧参考 `e24df2530ca770f78ebcda170ac55cc1203e4447` 与最新 base 的外部实验服务客户端是同一 blob；后续主要变化是工具注册、审批和显示属性。
...
| v2.5 base（描述性标签） | `f6ad20caf8963d105fe26c96b6a30ef4dfd34859` | 0/0 | 参考基线 |
| v2.5 evolve_tool（描述性标签） | `4ef5e0d354a0a24b6d2b2e1c8a8e2f5da2578c39` | 67/2 | 分叉 |
| v2.5 master-agent（描述性标签） | `3795cdcfc35cdc83f8f31873f9dc30b75a2e15da` | 67/1 | 分叉 |
| 记忆分支（描述性标签） | `dd4f542e6dc176915a18ed896158a2b2cece4728` | 94/0 | base 的祖先 |
...
完整包的注册和审批：`f6...:opencode/opencode.jsonc.template:267-283` 加载外部插件入口，三个 solver agent 为 `ask`，`evolve_create` 为 `ask`。外部插件内的 `tools/index.ts:20,29-36,50-52` 注册并检查 evolve 工具族，再交给 gate。lite 模板仍加载同一插件路径（`f6...:opencode_lite/opencode.jsonc.template:188-204`），但 lite 插件内的 `tools/index.ts:19-41` 明确省略 evolve；lite 的 tool approval 对象为空（同模板 `:193-200`），且不存在 lite 插件内的 `tools/evolve/` 树。
...
3. `882becbc2b1279ed91efc8352b906c1c60c823a9`：把快照里的 evolve 工具迁到 外部插件内的 `tools/evolve/tools/`，加入注册和审批。
4. `1e5a2ce9b5a14e699fab933f5a3c0826ad94275f`：v2.5 demo，删除旧生产 `opencode/plugins/evolution-loop.ts`、`tools/init_evolution.ts`、旧 evolution agents/command，并加入新的 solver 与外部插件路径。
5. `a5accdf854b5538b04dd80792ad4a0a2ea4a92ff`：为外部 benchmark 修复文件复制，将插件从 `opencode/` 的直接子目录整体移到 `opencode/tools/` 下；模板同步改路径。该提交的旧构建脚本仍只打包 `opencode/`（`9ee...:ipipe_build.sh:23-50`）。
6. `8f9f5c983fd6fd539597ae4a991bb74a2f4ab47f`：增加生产 外部插件内的 `tools/evolve/*`、注册表和 master hook。
7. `ce2dad9862d9fd250e1d00b244426c20ad97a688`：增加直接 CLI 防护与诊断；`4e572f62b6d8cc053be8621cb96c02ce5c235379`：补齐显示属性要求。

关键修正：`e24...` 与 `f6...` 的实验服务客户端为相同 blob `bc0d81938a319fbc42400830887c307517b1d931`。最新客户端（原文件行 62–79）读取显式配置的外部服务 URL 文件，再通过外部服务 CLI 传入 `--api-url ... --api-key ...` 调用；仓库没有证明远端算法、模型、预算或服务端版本。
...
- **evolve_tool (`4ef...`)**：提供七个控制工具的独立 overlay，写命令单次执行、失败为 unknown、不自动重试；构建到未跟踪的隐藏插件输出目录，需手工复制到目标项目，故不是自动进入 base 构建。
...
这份审计只做固定 Git blob、树和历史检查，没有运行 WebAgent、模型、远程参考引擎或公司评测平台，也没有读取运行时 URL 文件、密钥或部署仓库。仓库证据足以确定应优先研究 v2.5 base（描述性标签） 完整包的注册/审批和外部实验服务客户端调用链；不能把某个分支名、README 或本地快照当成已部署版本，也不能从代码直接归因某次分数或“无有效解”的原因。
