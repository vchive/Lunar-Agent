...
## Feature 113：当前版本真实多文件验收准备（2026-09-16）

正在独立登记 `c977eb4` 的两例真实 GLM-5.2 验收：预算选择与工作分配，各两份 JSON
输入、两份输出，一次尝试，串行执行。原生 `solve --evolve --multi-file` 使用两个初始
候选加一个后代；独立穷举最优值分别为 37、39，每例另有 12 个预先固定的 evaluator
核验样例。当前 CC Switch 默认模型不继承；显式 GLM-5.2 使用与 069/082 相同的网关。

登记源码、输入、模型、预算与 oracle 后先 commit/push，再执行。每例总时长 1200 秒、
单请求 180 秒、至多 16 次模型请求；160000 token 是已观察用量的停止阈值，不是服务端
硬消费上限。未知用量和请求失败后不再请求，不补槽、不回答澄清、不切模型。候选生成、
评测和交付仍使用冻结产品代码；测量层补跨角色用量日志和进程监督。

详见 `specs/113-real-multifile-acceptance/`。此准备记录不表示已经运行或获得有效解，
实际结果随后单独保存在该目录 postrun；旧 WebAgent 和历史失败测量保持冻结。
