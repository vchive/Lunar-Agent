# Implementation Plan: Frozen reference-benchmark Breakthrough Trial
...
`AlgorithmProblemContract`. The available external reference-benchmark result is instead a normal-Agent
experiment. WebAgent source confirms two distinct flows (role names below are descriptive;
original identifiers remain in the [historical archive](../../docs/history-archive.md)):

```text
normal: external master -> external build -> deliver
deep:   explicit /evolve -> external evolution coordinator -> external candidate builder -> outer continuation loop
```
...
new `lunar_evolution.effect_trial` module beside `lunar_evolution.benchmark` and never enters an evolution strategy.
...
|---|---|---|
| Standalone Distribution | Pass | Pure standard-library adapter; reference-benchmark tools remain optional external commands. |
