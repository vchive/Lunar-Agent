...
The inspected `reference-benchmark 1.10.6` experiment used 20 cases, three runs per case, and deep evolution
...
At inspected WebAgent `master` revision `0773e37` (original role filenames are retained in the
[historical archive](../../docs/history-archive.md)):

- the external master role delegates normal solving to the external build role and then delivers;
