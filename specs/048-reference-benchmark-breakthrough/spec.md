# Feature Specification: Frozen reference-benchmark Breakthrough Trial

**Feature Branch**: `048-reference-benchmark-breakthrough`
**Created**: 2026-09-04
**Status**: Implemented

## Context and scope

The available WebAgent experiment is a normal Agent run on `reference-benchmark 1.10.6`: 20 cases with
...
a bounded local trial for one or two frozen `reference-benchmark 1.10.6` cases. Each selected case is run
repeatedly in normal, non-evolution Lunar mode and scored through an explicit adapter to the exact
external reference-benchmark extractor/evaluator harness. A separately exported baseline supplies the historical
...
### User Story 2 — Repeat ordinary Lunar Evolution runs and use the official score boundary (P1)
...
- Bundling the external benchmark's private case material, FM-Eval service code, credentials, OpenCode, or WebAgent.
