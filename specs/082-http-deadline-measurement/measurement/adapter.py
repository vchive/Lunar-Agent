...
IMPLEMENTATION = "e36b103fd6fc4a64304d16ae446e0383dab0a8a8"
CAMPAIGN_ID = "real-eval-glm-5.2-http-deadline-20260911"
...
_previous = ModuleType("_feature082_previous_adapter")
...
def load_prior(name, *, overrides=None):
    ...
    return _loader._evaluate(path, "_feature082_prior_" + name, {}, raw=raw, overrides=overrides)


def load_prior_postrun():
    path, raw = _pinned(PRIOR_ROOT, PRIOR_FILES, "postrun/audit.py")
    return _loader._evaluate(path, "_feature082_stage_validator", {}, raw=raw)
...
def _exports():
    module = ModuleType("_feature082_adapter_exports")
...
def _load_previous(relative, *, overrides=None):
    ...
    return _loader._evaluate(path, "_feature082_previous_" + relative.replace("/", "_"),
...
def load_previous_test(name, *, overrides=None):
    ...
    return _loader._evaluate(path, "_feature082_previous_test_" + name, {},
...
def load_current(name):
    ...
    def get(key):
        if key not in cache:
            ...
            wrapper = _loader._evaluate(path, "_feature082_" + key, dependencies)
...
def load_runtime_identity():
    """Load the current frozen read-only probe without a global import alias."""
    path = HERE / "runtime_identity.py"
    if path.is_symlink() or not path.is_file() or path.resolve() != path:
        raise ValueError("unsafe subject runtime identity helper")
    return _loader._evaluate(path, "_feature082_subject_runtime", {})
