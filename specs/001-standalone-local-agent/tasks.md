# Tasks: Standalone Local Lunar Evolution Agent
...
- [x] T001 Create `pyproject.toml` with Python 3.11 metadata, `lunar_evolution` console entry point, and dev dependencies
- [x] T002 [P] Add repository `.gitignore` for virtual environments, local `.lunar-evolution/` state, secrets, and `.hermes/`
...
- [x] T004 Implement domain enums and dataclasses in `src/lunar_evolution/models.py`
- [x] T005 Implement SQLite schema, migrations, WAL mode, and idempotent events in `src/lunar_evolution/store.py`
- [x] T006 [P] Implement config/home/workspace resolution in `src/lunar_evolution/config.py`
- [x] T007 [P] Implement artifact path confinement and SHA-256 metadata in `src/lunar_evolution/artifacts.py`
...
- [x] T009 [P] [US1] Define Runtime Adapter protocol and result type in `src/lunar_evolution/runtime.py`
- [x] T010 [P] [US1] Implement deterministic `MockRuntime` in `src/lunar_evolution/runtime.py`
- [x] T011 [US1] Implement task claiming, retries, recovery, and terminal settlement in `src/lunar_evolution/controller.py`
- [x] T012 [US1] Implement `run`, `resume`, `status`, `events`, and `cancel` commands in `src/lunar_evolution/cli.py`
- [x] T013 [US1] Add module entry point in `src/lunar_evolution/__main__.py` and package metadata in `src/lunar_evolution/__init__.py`
...
- [x] T016 [P] [US2] Add dependency-aware task creation and ready-queue scheduling in `src/lunar_evolution/controller.py`
- [x] T017 [P] [US2] Add structured evaluator protocol and default acceptance evaluator in `src/lunar_evolution/evaluator.py`
- [x] T018 [US2] Add plan input format and CLI option in `src/lunar_evolution/cli.py`
...
- [x] T020 [P] [US3] Implement explicit `SubprocessRuntime` with timeout and stderr capture in `src/lunar_evolution/runtime.py`
...
- [x] T024 [P] Add JSON output mode and stable CLI exit-code documentation in `src/lunar_evolution/cli.py` and `specs/001-standalone-local-agent/contracts/runtime-adapter.md`
- [x] T025 Run the [quickstart](./quickstart.md), recovery suite, and static checks in CI configuration `.github/workflows/test.yml`
- [x] T026 Add `run --detach` durable-handle launch and coverage in `src/lunar_evolution/cli.py` and `tests/test_cli.py`
- [x] T027 Persist detached runner PID/PGID, terminate its process group on cancel, and discard late results in `src/lunar_evolution/controller.py` and `tests/test_plan.py`

## Phase 7: Built-in model runtime

- [x] T028 [US4] Implement dependency-free OpenAI-compatible HTTP Runtime with credential redaction in `src/lunar_evolution/runtime.py`
- [x] T029 [US4] Add explicit endpoint/model/API-key CLI and detached resume propagation in `src/lunar_evolution/cli.py`
...
2. Deliver P1 with the mock runtime and recovery tests; this is the first usable local Lunar Evolution Agent.
