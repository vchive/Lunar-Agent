"""Fixed two-slot corrected staged measurement, using the pinned native worker and receipt reader."""
...
def summarize():
    ...
    return {
        "schema_version": "1", "kind": "corrected_staged_measurement", "manifest_sha256": manifest_sha,
        "planned_attempts": 2, "all_slots_terminated": all(row["process_terminated"] for row in rows),
...
def launch():
    ...
    write_new(CAMPAIGN / "started.json", {
        ...
        "started_unix_seconds": time.time(), "planned_attempts": 2, "concurrency": 2,
    })
    loader = REPO / ".lunar/real-eval-20260908/ccswitch.py"
    spec = importlib.util.spec_from_file_location("measurement074_ccswitch", loader)
