"""Rebind the pinned two-slot fixtures; add only Feature 076 wrapper-specific regressions."""
import hashlib
...
def load(name):
    ...
    spec = importlib.util.spec_from_file_location("test_postrun076_" + name, path)
...
def report_fixture():
    ...
    fixture.update(kind="feature076_postrun_audit", implementation_commit=adapter.IMPLEMENTATION)
    return fixture
...
def test_current_campaign_source_and_analysis_identity_are_bound(top_level):
    ...
    assert result["kind"] == "feature076_postrun_audit"
    assert result["implementation_commit"] == "96d5a600f03e5401753efc6f579d2ff14fe839be"
...
def test_prior_campaign_cannot_be_audited_as_current(monkeypatch):
    ...
    old = adapter.REPO / "specs/074-corrected-staged-measurement/measurement/manifest.json"
    with pytest.raises(ValueError, match="wrong.*campaign"):
        audit.audit_registered_campaign(old, adapter.CAMPAIGN)


@pytest.mark.parametrize("field,value", [
    ("kind", "feature074_postrun_audit"), ("implementation_commit", "0" * 40),
])
...
def test_current_report_title_preserves_prior_score_and_denominator_contract():
    ...
    assert text.startswith("# Lunar Agent 公共计划交接评测（Feature 076）\n")
