...
```bash
uv run lunar-evolution init --home .lunar
mkdir -p demo
...
uv run lunar-evolution plan demo/plan.json \
  --runtime subprocess \
...
```bash
uv run lunar-evolution status <run-id> --json --home .lunar
uv run lunar-evolution events <run-id> --json --home .lunar
uv run lunar-evolution deliver <run-id> --json --home .lunar
```
