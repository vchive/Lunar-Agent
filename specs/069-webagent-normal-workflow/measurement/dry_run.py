...
TEST_MODULES = (
    "tests/test_dry_run069.py", "tests/test_worker069.py", "tests/test_staged_workflow.py",
    "tests/test_staged_effect_adapter.py", "tests/test_effect_adapters.py", "tests/test_effect_trial.py",
    "tests/test_campaign069.py",
)
SCENARIOS = {
    "control_adapter_and_native_trial_success": (
        "tests/test_dry_run069.py::test_native_offline_scenario[control_success]",
    ),
    "staged_adapter_and_native_trial_success_with_one_resume": (
        ...
        "tests/test_staged_workflow.py::test_durable_boundary_replays_pairs_with_same_identity_and_no_budget_reset",
    ),
...
def audit_registration(manifest_path: Path, campaign_path: Path) -> dict:
    ...
    spec = importlib.util.spec_from_file_location("measurement069_dry_run_audit", path)
...
def run_scenarios() -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="lunar-069-offline-") as temporary:
...
def dry_run(manifest_path: Path, campaign_path: Path) -> dict:
    ...
    return {
        "schema_version": "1", "kind": "feature069_offline_dry_run", "status": "passed",
