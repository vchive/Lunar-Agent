...
- [ ] T001 Define canonical audit-request, holdout-declaration, holdout-receipt, boundary-result,
      and combined-report schemas with bounded fields and private-field rejection.
- [ ] T002 Add fresh-identity and manifest-binding checks that cannot consume Feature 131/134/139
      identities as a new attempt and cannot be used as a launch preregistration.
- [ ] T003 Implement the read-only artifact/lifecycle adapter over
...
- [ ] T003a Check orchestration task ownership/discriminator, reciprocal child links, execution
...
- [ ] T004 Preserve native inode/device and snapshot semantics; report relocation, uncertainty,
      changed bytes, and missing receipts without repairing or reinterpreting them.
- [ ] T005 Implement canonical holdout declaration and receipt parsing with exact ordering,
...
- [ ] T006 Produce primary/joint eligibility projections, explicit no-side-effect flags, and
      first-problem reporting while leaving official acceptance counters unchanged.
- [ ] T007 Add provider-free positive and negative fixtures for each lifecycle boundary, holdout
      gaps/tampering, copied-directory inode mismatch, and audit reentrancy/read-only behavior.
- [ ] T008 Add an audit-only API/CLI surface only if it does not alter launch or continuation
      behavior; reject attempts to pass audit output as a launch manifest.
- [ ] T009 Run focused, shared, static, compile, SDD, and diff checks; document actual counts and
      known limitations. Do not run providers, campaigns, generated source, evaluators, or holdouts.
