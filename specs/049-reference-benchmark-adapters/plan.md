# Implementation Plan: Executable reference-benchmark Adapters
...
inspected external reference-benchmark cases expose `tests/extractor_agent.py` and `tests/evaluator.py`; FM-Eval's
...
1. **Adapters beside the protocol** — add one standard-library `lunar_evolution.effect_adapters` module. It
...
Feature 048 public subject request
        -> fresh Lunar Evolution loop -> solution files + score-free subject receipt
