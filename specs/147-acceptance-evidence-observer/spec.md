...
The observer validates the fixed registration identity, the six planned stage receipts
...
events are supplied. It returns a bounded projection with first missing/failed stage and `0/1` or
`1/1` preparation counters. Primary and joint remain `0/1`: structural closure cannot replace
...
counter remains `0/1` until a separate holdout auditor is implemented.
