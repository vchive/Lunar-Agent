...
```bash
lunar-evolution evolve contract.json --strategy loop --detach \
  --generator-command "/absolute/path/to/generator-wrapper" \
  --evaluator-command "/absolute/path/to/evaluator-wrapper" \
  --json --home .lunar
```

Resume with the same command/profile options. Lunar Evolution stores only SHA-256 fingerprints in
