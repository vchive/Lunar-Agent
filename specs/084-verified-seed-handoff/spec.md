...
famou-v2 starts with executable `initial_programs`, re-runs enrichment and evaluation locally, and refuses to start when no seed is usable. WebAgent 2.5 delegates deep evolution to a remote famou-v2 service through an experiment control plane. Lunar currently has a local population strategy, but it has no typed handoff for importing an externally produced candidate with its lineage, evaluator evidence, and environment assumptions. This feature adds that local handoff boundary and documents an opt-in remote backend contract without changing the staged Master→Build path or treating remote scores as Lunar scores.

### User Story 1 - Admit only locally verified seeds (Priority: P1)

An operator can place candidate artifacts and provenance in a local seed manifest. Lunar validates identity, source bounds, dependency metadata, and lineage, then runs the configured local evaluator. Only candidates with a fresh local evaluator receipt enter the initial population; an external score alone is never sufficient.
...
1. **Given** a bounded manifest whose source and dependencies are readable, **When** the local evaluator returns a valid receipt, **Then** the adapter emits one candidate with a stable ID, deterministic generation/iteration/island fields, and an evaluator receipt reference.
2. **Given** a manifest with a missing source, dependency failure, invalid receipt, or evaluator validity other than `1.0`, **When** it is imported, **Then** that seed is excluded with a fixed safe reason and no population update occurs.
3. **Given** that every seed is excluded or has no score, **When** initialization is validated, **Then** initialization fails closed and no generator, remote backend, or population mutation is made after the evaluator gate.
...
2. **Given** a changed source digest, dependency lock, evaluator fingerprint, or contract digest, **When** resume is attempted, **Then** it fails closed without adding a duplicate candidate.
...
### User Story 3 - Keep remote famou-v2 explicitly optional (Priority: P2)

An integration owner can describe a remote famou-v2 experiment using a typed submit/status/sync/continue/cancel boundary. The boundary records remote experiment identity and material provenance, but remote output is imported only as an untrusted seed candidate pending local exact-harness re-evaluation. The default CLI remains local and performs no network call.
...
- A seed whose dependency installation partially succeeds is not admitted; the receipt records a bounded failure code, never exception text or credentials.
...
- A remote candidate may be locally valid but still fail the current contract's exact harness; it is excluded from the population and remains provenance-only.

## Requirements

### Functional Requirements

- **FR-001**: System MUST accept a bounded local seed manifest containing candidate source, source digest, stable identity, lineage, dependency/environment metadata, and optional external evidence.
- **FR-002**: System MUST re-evaluate every imported seed with the configured local evaluator before creating a `Candidate` or changing population state.
- **FR-003**: System MUST require a structured local evaluator receipt containing contract digest, evaluator fingerprint, evaluator kind, validity, score fields, and receipt digest; remote imports MUST use the exact-harness evaluator kind, and external scores are advisory.
...
- **FR-005**: System MUST persist seed provenance and identity in candidate metadata and checkpoint/archive records, subject to existing bounded artifact and secret-safety limits.
- **FR-006**: System MUST reject resume when source, dependency/environment, evaluator, or contract identity no longer matches the persisted handoff.
...
- **FR-008**: System MUST expose a remote backend protocol for submit, status, sync, continue, and cancel with opaque IDs and bounded, credential-safe payloads.
- **FR-009**: System MUST keep the remote backend opt-in and absent from default staged and local evolution construction.
- **FR-010**: System MUST route every remote candidate through the configured exact-harness evaluator before local ranking, verify its evaluator fingerprint and receipt digest, and label remote scores as provenance-only until then.
...
- **SeedIdentity**: stable candidate ID derived from source, lineage, contract, dependency, and environment digests; collision-resistant and independent of external experiment IDs.
- **EvaluatorReceipt**: local authoritative validity/score evidence tied to contract and evaluator fingerprints.
- **SeedProvenance**: source kind (`local` or `famou-v2`), lineage, remote experiment ID if any, material references, and external evidence labels.
...
- **SC-004**: Remote fixture results with only external scores produce zero local scored candidates until exact local re-evaluation succeeds.

## Assumptions and limits

- The first implementation is local-only for seed admission; no real famou-v2 service is contacted.
