# Feature 117 validation

Product fixed to `9a26a73e38d52b18f003b262a8746bb22496c156`; no product change in this feature.
Its existing full regression is 5636 passed / 1 skipped. Only registration/measurement code and
documentation change. The new registration uses 600-second request/invocation/local-process and
3600-second task limits, retaining all other fixed 115 conditions.

## Prelaunch

Read-only provider inspection matches all safe 115 provider metadata; no credential value printed
and no model request made. Independent budget review confirmed runtime/native CLI/environment
propagation and clipping to remaining task time. Worker and response wrapper are byte-identical
to 115. Historical 113/115 and product bytes are unchanged.

The Feature 112 subprocess quickstart passed: independent scores 1, 2, 6, 7, one delivery copy,
and 1/1/1/4/4 contract/compiler/auditor/Agent/candidate counts unchanged after terminal resume.
Log: `/private/tmp/lunar117-quickstart.log`.

New registration/runner tests: **48 passed**. Combined old/shared/new measurement regression:
**216 passed in 3.84s**, zero failures/errors. Logs: `/private/tmp/lunar117-prelaunch.log` and
`/private/tmp/lunar117-prelaunch.xml`. Ruff, compileall, Specify and diff checks passed.

Registration SHA-256: `822d46f6cc9bfd0850129bf4455bd32eb4652b7f9a07f3dabe9b4a8066b43ec2`.
It binds 76 product files, 15 measurement/test files and 31 historical files. Preparation and
read-only verification completed with zero model calls; private campaign root did not yet exist.
Independent final review passed with no blocker: all 76/15/31 pins match, 14 fixed fields match
115, only the declared product/deadline changes differ, both worker/diagnostics copies are exact,
and the 600-second deadline propagates without a stale 180-second setting. Registration is
committed and pushed before any provider request; no slot can be reopened.

## Interpretation

This is a new /2 acceptance campaign. Both product state handling and time budgets changed since
