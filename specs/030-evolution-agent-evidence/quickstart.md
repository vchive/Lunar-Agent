...
Inspect the returned workspace with `lunar-evolution status RUN_ID --json` (or the equivalent status
command) and `lunar-evolution events RUN_ID --json`. Look for `evolution_agent_artifact`,
