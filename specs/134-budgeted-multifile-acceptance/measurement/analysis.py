...
from campaign import CAMPAIGN_ID, HERE, LIMITS, MANIFEST, REPO, sha, write_new
...
class ReadOnlyStore(Store):
    ...
    def _connect(self):
        ...
        with tempfile.TemporaryDirectory(prefix="lunar134-store-") as directory:
...
def usage_summary(path):
    ...
    for row in read_rows(path):
        ...
        if row.get("kind") == "request_started":
            ...
            cap = LIMITS["preparation_request_seconds"] if index in {2, 3} else LIMITS["candidate_seconds"]
            if timeout <= 0 or timeout > min(cap, LIMITS["request_seconds"]):
...
def request_failure_summary(product, usage, frozen):
    ...
    if (expected is None or usage["provider_requests"] != expected or usage["finished_requests"] != expected
            or usage["pending_requests"] or len(requests) != expected
            or any(row["index"] != index or row["stage"] != request_stage(index)
                   or row["outcome"] != "succeeded" for index, row in enumerate(requests[:-1], 1))):
        raise ValueError("request_failure_request_disagreement")
    failed = requests[-1]
    if (failed["index"] != expected or failed["stage"] != request_stage(expected)
...
def summarize(manifest):
    ...
    completed = (worker_status == "completed" and process_status == "exited"
