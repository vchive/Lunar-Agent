...
```bash
export LUNAR_EVOLUTION_MODEL_ENDPOINT='https://model.example/v1/chat/completions'
export LUNAR_EVOLUTION_API_KEY='provided-out-of-band'
...
declared runtime capabilities passed validation at that time. In this example,
`baseline-agentserver.json` is an owner-provided AgentServer/company-platform historical projection
for descriptive comparison; obtain and convert matching `webagent` evidence separately for any
WebAgent-specific claim. Generate a new kit at the example path when using the current adapter;
existing kits retain their original bytes and identities in the
