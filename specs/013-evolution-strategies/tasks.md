...
- [x] T002 [P] Add the strategy protocol, configuration validation, and shared result models in `src/lunar_evolution/evolution.py`.
- [x] T003 [P] Add the canonical evolution strategy contract to `specs/013-evolution-strategies/contracts/evolution-strategy.md`.
- [x] T004 Implement workspace-confined candidate persistence and atomic state replacement in `src/lunar_evolution/evolution.py`.
...
- [x] T006 [US1] Implement `LoopStrategy.run` and `LoopStrategy.resume` with fresh round contexts and append-only archive in `src/lunar_evolution/evolution.py`.
- [x] T007 [US1] Add loop strategy metadata to the algorithm workspace manifest in `src/lunar_evolution/algorithm.py` without changing legacy contracts.
...
- [x] T010 [US2] Implement `PopulationConfig`, `PopulationState`, objective-aware selection, token novelty, and active-set trimming in `src/lunar_evolution/evolution.py`.
- [x] T011 [US2] Implement optional island assignment and ring migration in `src/lunar_evolution/evolution.py`.
- [x] T012 [US2] Add population strategy parsing and bounds to `src/lunar_evolution/algorithm.py`, preserving the loop default and canonical contract digest.
...
- [x] T015 [US3] Implement `OpenEvolveStrategy` using an explicit argument list, bounded subprocess output, timeout, and result validation in `src/lunar_evolution/evolution.py`.
- [x] T016 [US3] Add optional `openevolve` strategy selection and a clear CLI/configuration error when no executable is supplied in `src/lunar_evolution/cli.py` and `src/lunar_evolution/algorithm.py`.
...
- [x] T019 [US4] Add an evolution command or controller entry point that selects a strategy while retaining SQLite run authority in `src/lunar_evolution/controller.py` and `src/lunar_evolution/cli.py`.
- [x] T020 [US4] Ensure cancellation, timeout, and recovery events preserve archive/state evidence in `src/lunar_evolution/controller.py` and `src/lunar_evolution/store.py`.
