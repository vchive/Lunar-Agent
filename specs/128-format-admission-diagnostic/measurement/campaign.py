...
CAMPAIGN_ID = "diagnostic128-glm-5.2-format-admission-20260917"
LIMITS = {"request_seconds": 600, "wall_seconds": 1320, "max_requests": 2,
          "token_stop_threshold": 160000, "holdout_seconds": 5}
REFERENCE = REPO / "specs/125-snapshot-protocol-diagnostic/measurement/manifest.json"
REFERENCE_SHA = "c76a7f4802ad2691fc0c400c3525f9e38f4c520399a1a6a9f8f8e4506ea43a12"
...
def fixed_conditions():
    ...
    with tempfile.TemporaryDirectory(prefix="lunar128-profile-") as directory:
    ...
    fixed = {
        ...
        "local_failure_capture": {
            "scope": "failed_preparation_only",
            "source": "exact_EvaluatorPreparationError_and_EvaluatorPreparationDiagnostic",
            "keys": ["schema_version", "stage", "reason", "probe_index", "input_index", "order_index"],
            "reader_validation": "failed_preparation_no_freeze_with_role_matching_accepted_requests",
            "missing_or_invalid_capture": None,
            "invalid_retained_detail": "reject_summary_without_publication",
    ...
    unchanged = (
        "schema_version", "contract", "contract_sha256", "input_utf8", "input_sha256", "input_profile",
        "holdouts", "limits", "runtime_options", "sampling", "provider", "planned_attempts",
        "schedule", "retries_or_replacements", "concurrency", "response_capture", "primary",
        "secondary", "joint", "failure_quality", "cost_micros", "limitations",
    )
    if any(fixed[field] != previous[field] for field in unchanged):
        raise ValueError("prior_conditions_changed")
    fixed["previous_campaign"] = {
        "campaign_id": previous["campaign_id"], "manifest_sha256": REFERENCE_SHA,
        "product_commit": previous["product_commit"],
        "compiler_request": previous["compiler_request"],
    }
    fixed["condition_changes"] = {
        field: {"previous": previous[field], "current": fixed[field]}
...
def measurement_paths():
    return [*HERE.glob("*.py"), *(HERE.parent / name for name in
            ...
            *REPO.glob("tests/test_measurement128_*.py")]


def historical_paths():
    return [p for number in (113, 115, 117, 120, 121, 122, 123, 124, 125, 126, 127)
