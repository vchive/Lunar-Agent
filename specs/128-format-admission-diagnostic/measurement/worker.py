...
from famou.evaluator_bundle import EvaluatorPreparationError, compile_evaluator_bundle
from famou.evaluator_diagnostics import EvaluatorPreparationDiagnostic
from famou.runtime import OpenAICompatibleRuntime


def local_failure_detail(exc, *, stage, frozen):
    """Project an exact typed local failure without changing its failed outcome."""
    try:
        if (stage == "preparation" and frozen is None
                and type(exc) is EvaluatorPreparationError
                and type(exc.diagnostic) is EvaluatorPreparationDiagnostic):
            return EvaluatorPreparationDiagnostic.from_dict(exc.diagnostic.to_dict()).to_dict()
    except Exception:  # noqa: BLE001 - optional detail cannot replace the original failure
        return None
    return None
...
def main():
    ...
    try:
        ...
        result["local_failure"] = local_failure_detail(exc, stage=stage, frozen=result["frozen"])
        code = 1
