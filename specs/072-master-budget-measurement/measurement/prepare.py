"""Materialize four budget-comparison inputs without invoking any model."""
...
POLICIES = {f'master_{seconds}': StagePolicy(seconds, 2400, 120, 32) for seconds in (300, 1200)}
LEGACY = REPO / 'specs/069-webagent-normal-workflow'
SEALED_PINS = {
    'measurement/manifest.json': '07781f3390586e49c2c521012e7981350c06215e6c7103a865a97f25597e423e',
    'measurement/prelaunch-audit.json': 'adbeb46bf746c329deea711e14dfc17ede44bcda91e947b215453f836fc9719c',
    'measurement/dry-run.json': '75acbe842b3c7286826d912840b6cbaa6e3482efb85e2e796830bbdc9b7ae4cc',
    'postrun/final-audit.json': '3175509fe7d2df7210047ec956ad2f24049189524adfe5670586704ec6cdb2e7',
    'postrun/final-report.md': 'b5c615539444985dfa28bb9fc1ab04908fd91b361421dbe14f2aa2dfaabe2a97',
}
...
def main():
    ...
    legacy_helpers = [*sorted((LEGACY / 'measurement').glob('*.py')),
                      LEGACY / 'postrun/audit.py', LEGACY / 'postrun/render_report.py']
    for path in legacy_helpers:
        relative = path.relative_to(REPO).as_posix()
        blob = subprocess.check_output(['git', 'show', f'{IMPLEMENTATION}:{relative}'], cwd=REPO)
        require(hashlib.sha256(blob).hexdigest() == sha(path), 'reused helper changed')
    for relative, digest in SEALED_PINS.items():
        require(sha(LEGACY / relative) == digest, 'sealed historical evidence changed')
    historical = {**read(LEGACY / 'measurement/manifest.json')['historical_files_sha256'],
                  **{(LEGACY / relative).relative_to(REPO).as_posix(): digest
                     for relative, digest in SEALED_PINS.items()}}
    require(all(sha(REPO / relative) == digest for relative, digest in historical.items()),
            'historical context bytes changed')
...
'''
    ...
    slots = []
    for index, (budget_arm, key) in enumerate(ORDER, 1):
    ...
    frozen_paths = [*sorted(inputs.glob('*.json')), CAMPAIGN / 'readiness.json',
                    ...
                    *legacy_helpers, *sorted((REPO / 'tests').rglob('*.py')),
                    REPO / '.lunar/real-eval-20260908/ccswitch.py',
    ...
    manifest = {
        ...
        'policies': {name: {'master_seconds': policy.master_seconds,
                           'build_seconds': policy.build_seconds,
                           'reserve_seconds': policy.reserve_seconds,
                           'checkpoint_after_rounds': policy.checkpoint_after_rounds}
        ...
        'limitations': ['descriptive_one_attempt_per_case_arm', 'not_webagent_reproduction',
                       'sealed_feature069_is_context_not_a_control',
                       'master_limit_changes_remaining_hints_and_downstream_budget',
