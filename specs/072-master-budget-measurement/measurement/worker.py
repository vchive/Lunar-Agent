"""Four staged subjects reuse the native worker with exact per-budget registration checks."""
...
def select_slot(manifest, index):
    ...
    require(isinstance(slots, list) and len(slots) == 4)
...
def verify_bindings(manifest_path, campaign, manifest, manifest_sha, slot, *, require_started=True):
    ...
    require(manifest["planned_attempts"] == 4 and manifest["execution"] == EXECUTION)
