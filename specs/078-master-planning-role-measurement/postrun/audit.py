"""Read-only Feature 078 audit through the pinned, explicitly rebound two-slot validator."""
...
_adapter = load_file("postrun078_adapter", MEASUREMENT / "adapter.py")
verify_registration = _adapter.load_previous_postrun().verify_registration


def dependencies():
    adapter = load_file("postrun078_current_adapter", MEASUREMENT / "adapter.py")
...
def audit_registered_campaign(manifest_path, campaign_path, *, require_complete=False):
    ...
    result["kind"] = "feature078_postrun_audit"
    return result
