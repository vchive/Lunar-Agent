...
IMPLEMENTATION = "fba6ab8cf5b27d3bd1b42f353907a6ae21c25ef9"
CAMPAIGN_ID = "real-eval-glm-5.2-master-role-20260910"
...
_previous = ModuleType("_feature078_previous_adapter")
...
def load_prior(name, *, overrides=None):
    ...
    return _loader._evaluate(path, "_feature078_prior_" + name, {}, raw=raw, overrides=overrides)


def load_prior_postrun():
    path, raw = _pinned(PRIOR_ROOT, PRIOR_FILES, "postrun/audit.py")
    return _loader._evaluate(path, "_feature078_stage_validator", {}, raw=raw)
...
def _exports():
    module = ModuleType("_feature078_adapter_exports")
...
def _load_previous(relative, *, overrides=None):
    ...
    return _loader._evaluate(path, "_feature078_previous_" + relative.replace("/", "_"),
...
def load_previous_test(name, *, overrides=None):
    ...
    return _loader._evaluate(path, "_feature078_previous_test_" + name, {},
...
def load_current(name):
    ...
    def get(key):
        if key not in cache:
            ...
            wrapper = _loader._evaluate(path, "_feature078_" + key, dependencies)
