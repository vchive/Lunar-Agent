"""Build a deterministic, public-only EffectTrial kit from local reference case bytes."""
...
from .effect_adapters import EffectAdapterError, benchmark_case_content_digest
...
def _inspect_case(key: str, source: Path) -> dict[str, object]:
    try:
        content_digest = benchmark_case_content_digest(source)
...
def build_effect_kit(
    ...
    *,
    benchmark_name: str = "reference-benchmark",
    evaluation_profile_name: str = "lunar-evolution-reference-default",
    ...
    try:
        ...
        for case in inspected:
            ...
            try:
                current_digest = benchmark_case_content_digest(case["source"])
            except EffectAdapterError as exc:
                raise EffectKitError(str(exc)) from exc
